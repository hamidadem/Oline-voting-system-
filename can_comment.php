...
$result=mysql_query("select * from candidate where c_id='$user_id'")or die(mysql_error);
...
<td colspan="2" bgcolor="#51a351" id="Menus" style="height:auto;border-radius:12px;">
		...
		<ul>
			...
			<li class="active"><a href="can_comment.php">Comment</a></li>
...
function isAlphabet(elem, helperMsg){
  ...
  <form id="form1" name="login" method="POST" action="can_comment.php" onsubmit='return formValidation()'>
 <div style="background-color:#51a351;border-radius:5px;font-family:Arial, Helvetica, sans-serif; color:#000000; padding:5px; height:22px;"> 
 
 
 <div style="float:left;" ><strong><font color="white" size="2px">Submit Comment</font></strong></div>
 <div style="float:right; margin-right:20px; background-color:#cccccc; width:25px;  text-align:center; border-radius:10px; height:12px;"><a href="candidate_view.php" title="Close"><img src="img/close_icon.gif"></a></div>
...
VALUES
('$_POST[user_id]','$_POST[fname]','$_POST[email]','$_POST[com]','$date','unread')";
...
if (!mysql_query($sql,$conn))
  ...
  die('Error: ' . mysql_error());
         ...
         echo' <meta content="5;can_comment.php" http-equiv="refresh" />'; 
		 }
mysql_close($conn)
?>
