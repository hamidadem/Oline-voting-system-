...
<select required x-moz-errormessage="Select candidate" name="candidate" disabled="">
	<?php $CYS_query=mysql_query("select * from candidate")or die(mysql_error());
...
if(isset($_POST['ok']))
 ...
 $candidate=$_POST['candidate'];
	$query="SELECT * FROM voter where vid='$user_id'";
...
{
$sql="INSERT INTO result (vid,fname,mname,station,choice)
VALUES
('$user_id','$FirstName','$middleName','$station','$candidate')";
  $update = mysql_query("update voter set status=1
  WHERE vid='{$user_id}'") or die(mysql_error());
